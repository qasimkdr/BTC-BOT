...
export default function V3TradingAgentsCard(){
 ...
 return <section className="strategy-lab glass-panel">
  <div className="strategy-lab-head"><div><div className="eyebrow">V3 TRADINGAGENTS</div><h2>AI Shadow Committee</h2><p>Full analyst → debate → trader → risk → reflection workflow. Observation only; V2 execution remains untouched.</p></div><span className="run-backtest-btn" style={{cursor:"default"}}>{!data?.llmConfigured?"AI KEY REQUIRED":data?.shadowEnabled?"SHADOW RUNNING":"SHADOW OFF"}</span></div>
  {err&&<div className="lab-error">{err}</div>}
  {!x?<div className="lab-idle"><div className="orbital-loader"><span/></div><div><strong>No V3 decision yet</strong><p>{!data?.llmConfigured?"Configure the V3 LLM provider on the backend.":!data?.shadowEnabled?"V3 is configured but the shadow scanner is disabled.":"Waiting for the first completed 15m evaluation."}</p></div></div>:
